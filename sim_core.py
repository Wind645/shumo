...
def run_problem1(
    ...
    with torch.no_grad():
        fake_target = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float)
        m1 = Missile(pos0=torch.tensor([20000.0, 0.0, 2000.0]), speed=300.0, target=fake_target)
        fy1_pos0 = torch.tensor([17800.0, 0.0, 1800.0], dtype=torch.float)
        dir_h = fake_target.clone(); dir_h[2] = fy1_pos0[2]
