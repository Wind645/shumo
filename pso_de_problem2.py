...
"""
Hybrid Particle Swarm Optimization (PSO) + Differential Evolution (DE) for Problem 2
...
Usage:
  python pso_de_problem2.py
  python pso_de_problem2.py --use-cli --iters 300 --swarm 50 --method judge_caps --seed 42
...
CONFIG = dict(
    ...
    # DE hybrid (applied each iteration on a fraction of particles)
    de_fraction=0.35,       # fraction of particles to attempt DE trial on
    de_strategy="rand1bin", # 'rand1bin' | 'best1bin'
    de_F=0.7,
    de_CR=0.9,
    ...
    verbose=False,
    checkpoint_file="best_p2_pso_de.json",
    ckpt_every_iter=10,
)
# ==================================================================
...
def _de_mutate_rand1(rng: np.random.Generator, X: np.ndarray, F: float) -> np.ndarray:
    n = len(X)
    r1, r2, r3 = rng.choice(n, size=3, replace=False)
    return vec_add_angle(X[r1], F * vec_sub(X[r2], X[r3]))


def _de_mutate_best1(rng: np.random.Generator, X: np.ndarray, best: np.ndarray, F: float) -> np.ndarray:
    n = len(X)
    r1, r2 = rng.choice(n, size=2, replace=False)
    return vec_add_angle(best, F * vec_sub(X[r1], X[r2]))


def _bin_crossover(rng: np.random.Generator, target: np.ndarray, mutant: np.ndarray, CR: float) -> np.ndarray:
    D = target.shape[0]
    ...
    trial[1] = wrap_angle(trial[1])
    return trial


def pso_de(args: SimpleNamespace):
    ...
    if args.checkpoint_file:
        _atomic_save_json(gbest.as_dict() | dict(iter=0, timestamp=time.time()), args.checkpoint_file)

    X = np.stack([p.x for p in swarm], axis=0)
    ...
    for it in range(1, args.iters + 1):
        ...
        # DE refinement on a subset
        m = max(1, int(args.de_fraction * args.swarm_size))
        idxs = rng.choice(args.swarm_size, size=m, replace=False)
        X = np.stack([p.x for p in swarm], axis=0)
        for i in idxs:
            target = swarm[i]
            if args.de_strategy == "best1bin":
                mutant = _de_mutate_best1(rng, X, gbest.x, args.de_F)
            else:
                mutant = _de_mutate_rand1(rng, X, args.de_F)
            trial = _bin_crossover(rng, target.x, mutant, args.de_CR)
            trial = _clip_and_wrap(trial, lo, hi)
            f_trial = _eval_vec(trial, args.method, args.dt)
            if f_trial >= target.f:
                target.x = trial
                target.f = f_trial
                if f_trial >= target.pbest_f:
                    target.pbest_x = trial.copy()
                    target.pbest_f = f_trial
                if f_trial > gbest.f:
                    gbest = Particle(x=trial.copy(), v=target.v.copy(), f=f_trial,
                                     pbest_x=target.pbest_x.copy(), pbest_f=target.pbest_f)
        ...
        if args.ckpt_every_iter and (it % args.ckpt_every_iter == 0) and args.checkpoint_file:
            _atomic_save_json(gbest.as_dict() | dict(iter=it, timestamp=time.time()), args.checkpoint_file)

    if args.checkpoint_file:
        _atomic_save_json(gbest.as_dict() | dict(iter=args.iters, timestamp=time.time()), args.checkpoint_file)
...
def _build_args_via_config() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        c1=cfg['c1'], c2=cfg['c2'], v_clamp_frac=cfg['v_clamp_frac'],
        de_fraction=cfg['de_fraction'], de_strategy=cfg['de_strategy'], de_F=cfg['de_F'], de_CR=cfg['de_CR'],
...
def main():
    ...
    if not use_cli:
        ...
        import argparse
        ap = argparse.ArgumentParser(description="PSO + DE for Problem 2 (maximize M1 occlusion)")
        ...
        ap.add_argument('--de-fraction', type=float, default=0.35)
        ap.add_argument('--de-strategy', choices=['rand1bin', 'best1bin'], default='rand1bin')
        ...
        ap.add_argument('--checkpoint-file', type=str, default='best_p2_pso_de.json')
